...
Updated on Thu Apr 12 2023
@author: dsetiaputra

This script is compatible with the output of ColabFold v1.4 due to this version's naming scheme.
...
#Contact probability as defined by Humphreys Science 2021
#Interpret as the number of bins size 0.5A between 2-12A out of total bins (21) is in the top right quadrant of the pae
#Then take the highest contact probability for a residue pair out of these
def contact_prob(len_ch1, paes):
    error = np.array(paes)
    rounded_errors = np.round(error.astype(np.float64), decimals=1)
    error_TR = rounded_errors[len_ch1:,0:len_ch1].transpose()
    #Anything below 2 will be set to 1.99
    error_TR[error_TR < 2] = 1.99
    #Round down to the nearest 0.5
    error_TR_trunc = np.trunc(error_TR*2)/2
    #Get the contact probability (sum of probabilities of bins from 2-12, 0.5A bins; 21 bins total)
    error_TR_binned = (12 - error_TR_trunc)*2/21
    #If negative number, set to 0
    error_TR_binned[error_TR_binned < 0] = 0
    return(np.max(error_TR_binned))
