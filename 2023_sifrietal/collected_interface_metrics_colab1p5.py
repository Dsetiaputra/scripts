...
This script is compatible with the output of ColabFold v1.5 due to this version's naming scheme.
...
scores_files = [file for file in all_files if "scores" in file]
...
for run in prefix_lst :
    ...
    for rank in range(1,len(model_pdbs)+1) :
        ...
        current_pdb = [file for file in model_pdbs if "rank_00"+str(rank) in file][0]
        ...
        #Get iptm score
        current_scores = [file for file in model_scores if "rank_00"+str(rank) in file][0]
        ...
        out_line= run.replace("\\","") +ods+str(rank)+ods+str(avg_if_pae)+ods+str(avg_if_plddt)+ods+str(pdockq)+ods+str(iptm)+"\n"
